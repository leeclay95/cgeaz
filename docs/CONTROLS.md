...
| `cge-audit-storage-shared-key` (Audit; my own policy, added) | Flags storage accounts that allow shared-key access: identity or nothing for data stores. Audit, not Deny, because it is a new control and the Functions runtime scratch accounts keep shared keys by documented exception; carries its 800-53 and CSF mapping in the definition's metadata | IA-5, AC-3 | PR.AA-05 |
...
| `cge-audit-storage-shared-key` (Audit) | Nothing is blocked; storage accounts that allow shared-key access are marked non-compliant. Expect the two Functions scratch accounts to show as non-compliant: they need shared keys for zip deploys and timers | Compliance noise only | Set `shared_key_policy_effect` to `Disabled` through a reviewed change |
